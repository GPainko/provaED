...
Arvore *excluir(int valor, Arvore *raiz) {
    if (raiz) {
        if (raiz->valor == raiz->valor) { //valor localizado e pronto pra exclusão
            //eh folha
            ...
            if (raiz->valor < raiz->valor) { //avancar para esquerda
...
int localizarNivelmaioremenor(int valor, Arvore *raiz) {
  Arvore *p;
  int numeromenor = menorElemento(raiz)->valor;
  int numeromaior = maiorElemento(raiz)->valor;
    if(localizarNivel(numeromenor, raiz) == localizarNivel(numeromaior, raiz)){
      return 0;
    }else{
      return 1;
    }
